...
std::optional<Error> DhcpServer::send_to(netif *nif, const void *buffer, size_t len, uint32_t ip, uint16_t port)
{
    ...
    ip_addr_t dest;
    IP4_ADDR(ip_2_ip4(&dest), static_cast<uint8_t>((ip >> 24) & 0xff), static_cast<uint8_t>((ip >> 16) & 0xff),
...
void DhcpServer::handle_request(udp_pcb *upcb, pbuf *p, const ip_addr_t *src_addr, u16_t src_port)
{
    ...
    case DHCPREQUEST:
    {
        ...
        leases[yi].expiry = static_cast<uint16_t>((cyw43_hal_ticks_ms() + DEFAULT_LEASE_TIME_S * 1000) >> 16);
    ...
    *opt++ = DHCP_OPT_END;
    ...
    (void)send_to(nif, &dhcp_msg, static_cast<size_t>(opt - reinterpret_cast<uint8_t *>(&dhcp_msg)), 0xffffffff, PORT_DHCP_CLIENT);
