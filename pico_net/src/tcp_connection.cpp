...
void TcpConnection::handle_tcp_err(err_t err)
{
    ...
    {
    case ERR_ABRT:
        last_error_message = "tcp aborted";
        break;
    case ERR_RST:
        last_error_message = "tcp reset";
        break;
    case ERR_CLSD:
        last_error_message = "tcp closed";
        break;
    case ERR_CONN:
        last_error_message = "tcp connection error";
        break;
    case ERR_TIMEOUT:
        last_error_message = "tcp timeout";
        break;
    default:
        last_error_message = "tcp error";
        break;
    }
}
...
std::tuple<int, std::optional<Error>> TcpConnection::write(const std::byte *buffer, size_t size)
{
    ...
    err_t err = tcp_write(pcb, buffer, to_write, TCP_WRITE_FLAG_COPY);
    if (err == ERR_OK)
    {
        err = tcp_output(pcb);
    }
