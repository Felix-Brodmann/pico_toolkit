...
void UdpConnection::udp_recv_callback(void *arg, udp_pcb *pcb, pbuf *p, const ip_addr_t *addr, u16_t port)
{
    if (arg == nullptr)
    {
        if (p != nullptr)
...
std::tuple<size_t, std::optional<Error>> UdpConnection::write(const std::byte *buffer, size_t size)
{
