 ...
 * @details This class encapsulates a UDP connection, providing methods for reading, writing, and managing the connection. It uses the lwIP TCP/IP
 * stack for handling UDP connections. Since UDP is connectionless, this class manages the remote address for sending and receiving datagrams, and it
 * can be associated with a UdpListener for receiving datagrams from multiple remote endpoints.
 * @ingroup pico_net
 */
...
private:
    ...
    UdpConnection(udp_pcb *pcb, Address local_addr, Address remote_addr, bool owns_pcb = true, UdpListener *listener_owner = nullptr)
        : pcb(pcb), local_addr(std::move(local_addr)), remote_addr(std::move(remote_addr)), owns_pcb(owns_pcb), listener_owner(listener_owner)
    {
    }
